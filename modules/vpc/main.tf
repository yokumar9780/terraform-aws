...
resource "aws_subnet" "public" {
  ...
  tags = "${merge(map("Name", format("%s-${var.public_subnet_suffix}-%s", var.name, element(var.availability_zones, count.index))), var.tags, var.public_subnet_tags)}"
}
...
resource "aws_subnet" "private" {
  ...
  tags = "${merge(map("Name", format("%s-${var.private_subnet_suffix}-%s", var.name, element(var.availability_zones, count.index))), var.tags, var.private_subnet_tags)}"
}
...
resource "aws_eip" "nat" {
  ...
  vpc = true
  tags = "${merge(map("Name", format("%s-%s", var.name, element(var.availability_zones, (var.single_nat_gateway ? 0 : count.index)))), var.tags, var.nat_eip_tags)}"
}
...
resource "aws_nat_gateway" "this" {
  ...
  tags = "${merge(map("Name", format("%s-%s", var.name, element(var.availability_zones, (var.single_nat_gateway ? 0 : count.index)))), var.tags, var.nat_gateway_tags)}"
